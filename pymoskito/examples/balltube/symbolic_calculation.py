...
A_cross = np.concatenate((np.concatenate((A_num, C_num), axis=0),
                          np.zeros((A_num.shape[0] + C_num.shape[0], C_num.shape[0]))), axis=1)
B_cross = np.concatenate((B_num, np.zeros((C_num.shape[0], B_num.shape[1]))), axis=0)
C_cross = np.concatenate((C_num, np.zeros((C_num.shape[0], C_num.shape[0]))), axis=1)
Q_c_cross = np.concatenate((B_num,
                            A_num @ B_num,
                            np.linalg.matrix_power(A_num, 2) @ B_num,
                            np.linalg.matrix_power(A_num, 3) @ B_num,
                            np.linalg.matrix_power(A_num, 4) @ B_num), axis=1)
n = np.rank(Q_c_cross)  # system is not controllable, because n=2 < 5 system order
...
L_ball = T * L_star
L_ball_func = sp.lambdify((c0, c1, c2, x1_d, x1_d_dot, x4_d, x4_d_dot, k_L, k_V, A_B, A_Sp, m, g), L_ball,
                          modules=mat2array)
