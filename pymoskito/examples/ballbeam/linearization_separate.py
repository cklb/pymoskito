# -*- coding: utf-8 -*-
"""
Linearization of the Ball and beam system
"""

import sympy as sp
from sympy import sin, cos, Matrix

params = sp.symbols('s, x, x1, x2, x3, x4, x01, x02, x03, x04, u1, tau, tau0, '
                    'M , G , J , J_ball , R, B')
s, x, x1, x2, x3, x4, x01, x02, x03, x04, u1, tau, tau0, M, G, J, J_ball, \
R, B = params

# f1 = ddr
f1 = B * (x1 * x4 ** 2 - G * sin(x3))
# f2 = ddtheta
f2 = (tau - 2 * M * x1 * x2 * x4 - M * G * x1 * cos(x3)) / (M * x1 ** 2 + J
                                                            + J_ball)
...
def calcFeedbackGain(A, B, poles):
    ...
    K = t1T * (
    A ** 4 + p[3] * A ** (3) + p[2] * A ** 2 + p[1] * A + p[0] * sp.eye(4))
    return K
