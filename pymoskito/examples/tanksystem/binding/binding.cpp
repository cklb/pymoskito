...
PYBIND11_MODULE(Controller, m)
{
    ...
    .def(py::init())
    .def("create", &PIDController::create, "Create the controller with all needed constants and sets the integral and last error variable to zero")
    ...
    .def("getLastError", &PIDController::getLastError, "Returns the the given error element from the last compution");
}